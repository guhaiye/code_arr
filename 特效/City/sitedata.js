...
strVarCity += "                            <ul>";
strVarCity += "                              <li onclick=\"removenode(this)\" data-selector=\"tab-all\" class=\"active\"><a href=\"javascript:;\"><span>全部<\/span><em><\/em><\/a><\/li>";
strVarCity += "                            <\/ul>";
strVarCity += "                          <\/div>";
strVarCity += "                          <div class=\"data-container data-container-0\">";
...
function selectProvince(type,con,isremove){
		 ...
		 if(type=="all"){
		 	...
		 	strVarCity += "                                <\/ul>";
		 	strVarCity += "                              <\/div>";
		 	$('.data-container-0').html(strVarCity);
		...
		else
		{
			...
			if(typeof(relations[$(con).data("code")])!="undefined")
			{
				...
				//添加标题
				if(isremove!="remove"){
					$('.data-tabs ul').append('<li data-code='+$(con).data("code")+' data-name='+$(con).data("name")+' class="active" onclick="removenode(this)"><a href="javascript:;"><span>'+$(con).data("name")+'</span><em></em></a></li>');
				}
				//添加内容
				strVarCity += "<ul class=\"clearfix\">";
				strVarCity+='<li class="" style="width:100%"><a href="javascript:;" class="d-item data-all"  data-code="'+$(con).data("code")+'"  data-name=\"'+$(con).data("name")+'\"  onclick="selectitem(this)">'+$(con).data("name")+'<label>0</label></a></li>';				
			...
			}else
			{
				...
				{
					$('.data-result').append("<span class=\"svae_box aui-titlespan\" data-code=\""+$(con).data("code")+"\" data-name=\""+$(con).data("name")+"\" onclick=\"removespan(this)\">"+$(con).data("name")+"<i>×<\/i><\/span>");
