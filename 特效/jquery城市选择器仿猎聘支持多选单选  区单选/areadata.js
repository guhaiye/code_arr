...
strVarCity += "                          <div class=\"data-result\"><em>最多选择 <strong>35<\/strong> 项<\/em><\/div>";
strVarCity += "                          <div class=\"data-error\" style=\"display: none;\">最多只能选择 35 项<\/div>";
    ...
    if (type == "all") {
        ...
        $('.data-result span').each(function(index) {
            if ($('a[data-code=' + $(this).data("code") + ']').length > 0) {
                ...
                provincenum=parseInt(numlabel) + 1; 
            }
        });
    }
    //显示下一级
    else {
        var nextlevel=provincenum;
        var dataCityxz = __LocalDataCities.category.provinces;
        ...
        $('.data-result span').each(function(index) {
            if ($('a[data-code=' + $(this).data("code") + ']').length > 0) {
                ...
                // var numlabel=$('a[data-code='+$(this).data("code").toString().substring(0,3)+']').find('label').text();
                $('a[data-code=' + $(this).data("code").toString().substring(0, 6) + ']').find('label').text('1').show();
            }
        });
        //这是让区单选
        var datacodelen=$(con).attr('data-code').length;
        if(datacodelen==9){
           $(con).parent().parent().find('.data-all').find('label').text('1').show();
           var datacodesub=$(con).attr('data-code').toString().substring(0,6);
           var datacodestring=$(con).attr('data-code');
           var datacodename=$(con).parent().parent().find('.data-all').attr('data-name');
           $('.data-result span').each(function(index) {
              var datacodspanlen=$(this).attr('data-code').length;
              if(datacodspanlen==9){
                 var datacodspansub=$(this).attr('data-code').toString().substring(0,6);
                 if(datacodesub==datacodspansub){
                   var datacodespanstring=$(this).attr('data-code');
                   $('.data-result span[data-code="' + datacodespanstring + '"]').remove();
                   $('a[data-code="' + datacodespanstring + '"]').removeClass('d-item-active');
                   $('a[data-code="' + datacodestring + '"]').addClass('d-item-active');
                   $('.data-result').append("<span class=\"save_box aui-titlespan\" data-code=\"" + $(con).data("code") + "\" data-name=\"" +datacodename +'-' +$(con).data("name") + "\" onclick=\"removespan_area(this)\">" +datacodename +'-' + $(con).data("name") + "<i>×<\/i><\/span>");
                 }
              }
           });           
        }
...
function save_City() {
    ...
    if ($('.save_box').length > 0) {
        $('.save_box').each(function() {
            ...
            jobid+=$(this).data("code").toString().substring(-3)+',';
        })
    }
    if (val != '') {
